...
object Ex11 extends App{
  ...
  /*after you process
  the current message within react( ), you’d have to call other methods at
  the end of your message processing. Scala can pretty much hand over
    the execution of that call to any thread from the thread pool. Let’s take
    a look at this behavior with an example:*/
