...
object WordCount {
  def main(args: Array[String]) {
    val ipFile = "/home/edureka/Downloads/Server_log_Sample/hellospark_ip" // Should be some file on your local system
    ...
    val op1 = ip.flatMap(line => line.split(" ")).filter(_.contains("Vishal")).map(word=>(word,1)).reduceByKey((a,b)=>a+b)
    op1.foreach(println)
    val op2=op1.count()
    println("count = "+op2)
