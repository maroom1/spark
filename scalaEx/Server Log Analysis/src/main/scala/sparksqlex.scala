...
object RDDRelation {
 ...
 println(" testing dataframe done")
    // Write out an RDD as a parquet file.
    df.write.mode("Append").parquet("pair.parquet")
...
sqlContext.sql("SELECT count(*) FROM parquetFile").collect().foreach(println).toString().replaceAll("]","").replaceAll("[", "")
    sc.stop()
  }
}
