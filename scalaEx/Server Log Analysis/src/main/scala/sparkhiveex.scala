...
object HiveFromSpark {
  ...
  //  hiveContext.cacheTable("oitemTab")	
   ...
   // sql("SELECT COUNT(*) FROM orderTab where bid>5000").saveAsParquetFile(output+"/bidgt5000/")
  //  println(s"agg COUNT(*) bid>5000: $cnt")	
		
    //cnt=sql("SELECT * FROM oitemTab WHERE price>500").count()	
   // sql("SELECT * FROM oitemTab WHERE price>500").saveAsParquetFile(output+"/pricegt500/")
   // println(s"select COUNT(*) price>250: $cnt")
	
		//cnt=sql("SELECT * FROM oitemTab WHERE price>1000").count()	
    //sql("SELECT * FROM oitemTab WHERE price>1000").saveAsParquetFile(output+"/pricegt1000/")
    //println(s"select COUNT(*) price>750: $cnt")
	
    //cnt=sql("SELECT * FROM orderTab r JOIN oitemTab s ON r.oid = s.oid").count()	
	  
    sql("SELECT * FROM orderTab r JOIN oitemTab s ON r.oid = s.oid").saveAsTable("jointab")
...
/*
 * for this I got an error unable to initilize ... perm size error 
 * 
 * so try to change the 
 * VM Arguments in RUN configuration arguments 
 * 
 * -XX:PermSize=512M -XX:MaxPermSize=1024M
 * 
 * 
 * the arguments for this program is 
 * 
 * "/home/edureka/Downloads/Server Log Analysis/" "/home/edureka/Downloads/Server Log Analysis/output" 2
 * 
 * 
 * 16/06/02 13:23:57 INFO ParseDriver: Parsing command: SELECT * FROM orderTab r JOIN oitemTab s ON r.oid = s.oid
16/06/02 13:23:58 INFO ParseDriver: Parse Completed
Exception in thread "main" org.apache.spark.sql.AnalysisException: Duplicate column(s) : "oid" found, cannot save to parquet format;
